...
function findByName(items, needle) {
    ...
    return (
        items.find((x) => String(x?.name || "").toLowerCase().includes(n)) ||
        items.find((x) => String(x?.shortName || "").toLowerCase().includes(n)) ||
        items.find((x) => String(x?.description || "").toLowerCase().includes(n)) ||
        null
    );
}
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    if (type === "ROPRIME_REQUEST_MANAGEMENT") {
        chrome.permissions
            ...
            .then((granted) => sendResponse({ ok: true, granted }))
            .catch((err) => sendResponse({ ok: false, error: asLastErrorMessage() || String(err || "") }));
        return true;
    }
    ...
    if (type === "ROPRIME_GET_INSTALLED_EXTENSIONS") {
        containsManagementPermission()
            .then((granted) => {
                if (!granted) return sendResponse({ ok: false, error: "missing_management_permission" });
        ...
        containsManagementPermission()
            .then((granted) => {
                ...
                chrome.management.getAll((items) => {
                    ...
                    const robloxqolOut = robloxqol ? normalizeExtensionItem(robloxqol) : null;
                    sendResponse({
                        ok: true,
                        rovalra: rovalraOut,
                        roseal: rosealOut,
    ...
    if (type === "ROPRIME_SET_EXTENSION_ENABLED") {
        ...
        containsManagementPermission()
            .then((granted) => {
                if (!granted) return sendResponse({ ok: false, error: "missing_management_permission" });
                if (!id) return sendResponse({ ok: false, error: "missing_extension_id" });
