...
function writeDistManifest(platform, platformDistDir) {
    ...
    for (const entry of manifest.content_scripts || []) {
        if (Array.isArray(entry.ts)) {
            entry.ts = entry.ts.map((p) => typeof p === 'string' ? p.replace(/^\/?dist\//, '') : p)
        }
...
function copyBackgroundToPlatform(platformDistDir) {
    const src = join(bundleDir, 'background.ts')
    if (!existsSync(src)) {
        throw new Error('Missing bundled background.ts after esbuild.')
    }
    cpSync(src, join(platformDistDir, 'background.js'))
    const mapSrc = join(bundleDir, 'background.js.map')
    if (existsSync(mapSrc)) {
        cpSync(mapSrc, join(platformDistDir, 'background.ts.map'))
    }
}
...
await bundleEntry(
    ...
    join(bundleDir, 'background.ts'),
    'iife',
)
await bundleEntry(
    'src/content/profile/avatarPreview.ts',
