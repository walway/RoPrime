(() => {
	...
	var RP_FRIEND_STYLING_REIMAGNED_STYLE_ID =
...
`.trim();
	...
	function isRobloxHomePage() {
		...
		return (
			parts.length >= 1 && parts[parts.length - 1].toLowerCase() === "home"
		);
	}
    ...
    `;
		...
		root
			.querySelector(".roprime-welcome-ok")
	...
	function getRobloxAccountMenu() {
		...
		const menu = menus.find((m) =>
			m.querySelector(
				'.menu-option a[href*="browser-preferences"], .menu-option a[href*="browserpreferences"]',
			),
		);
	...
	function injectRoEliteTab(onNavigate) {
		...
		const insertAfter =
			menu
				.querySelector('.menu-option a[href*="browser-preferences"]')
				?.closest(".menu-option") ||
			menu
				.querySelector('.menu-option a[href*="browserpreferences"]')
				?.closest(".menu-option") ||
			null;
	...
	function ensureAccountDivider() {
		...
		const existingDividers = Array.from(
			menu.querySelectorAll('li[data-roprime-contentjs-account-divider="1"]'),
		);
		const divider =
			existingDividers[0] instanceof HTMLElement ? existingDividers[0] : null;
		existingDividers.slice(1).forEach((el) => {
			el.remove();
		});
		const browserPreferencesItem =
			...
			null;
		const desiredAnchor =
			browserPreferencesItem instanceof HTMLElement
				? browserPreferencesItem
				: oldRobloxTab;
		const desiredPosition =
			desiredAnchor === oldRobloxTab ? "beforebegin" : "afterend";
		const activeDivider =
			divider ||
			(() => {
		...
		const currentAnchor =
			activeDivider.previousElementSibling === desiredAnchor ||
			activeDivider.nextElementSibling === desiredAnchor
				? desiredAnchor
				: null;
		if (!currentAnchor)
		...
		while (collapsed) {
			...
			for (let i = 0; i < ch.length - 1; i++) {
				const a = ch[i];
				const b = ch[i + 1];
				if (!isThickRbx(a) || !isThickRbx(b)) continue;
				const aOurs =
					a.getAttribute("data-roprime-contentjs-account-divider") === "1";
				const bOurs =
					b.getAttribute("data-roprime-contentjs-account-divider") === "1";
	...
	function removeRoPrimeAccountUi() {
		...
		document
			.querySelectorAll('li[data-roprime-contentjs-account-divider="1"]')
			.forEach((divider) => {
				divider.remove();
			});
	}
...
`.trim();
	...
	function syncSidebarCompactBottomButton() {
		...
		const listCandidates = Array.from(nav.querySelectorAll("ul, ol")).filter(
	...
	var FALLBACK_LANGUAGE_KEYS = {
		en: {
			...
			"settings.oldNav.desc":
				"In development. Keep disabled unless you are testing it.",
			"settings.sidebar.title": "Sidebar size",
			"settings.sidebar.desc":
				"Drag freely, then release to snap to the nearest mode.",
			...
			"settings.friend.desc":
				"Restyles the friends carousel with a dark glass card and animated presence ring glow.",
		...
		ru: {
			...
			"settings.sidebar.desc":
				"Перетяните ползунок и отпустите для выбора ближайшего режима.",
			...
			"settings.friend.desc":
				"Обновляет карусель друзей: темная карточка и анимированное свечение статуса.",
	...
	function getLanguageLabel(langCode) {
		const dict =
			languageKeysCache?.[getLanguageCode()] ||
			FALLBACK_LANGUAGE_KEYS[getLanguageCode()];
	...
	async function ensureLanguageKeys() {
		...
		languageKeysPromise = (async () => {
			try {
				if (typeof chrome !== "undefined" && chrome.runtime?.getURL) {
					const enUrl = chrome.runtime.getURL(
						".locales/en/translation-keys.json",
					);
					const ruUrl = chrome.runtime.getURL(
	...
	function getLevenshteinDistance(a, b) {
		...
		const matrix = Array.from({ length: b.length + 1 }, (_, row) =>
			Array.from({ length: a.length + 1 }, (_2, col) =>
	...
	function buildSearchHighlightFragment(sourceText, matcher) {
		...
		while (true) {
			...
			if (matchIndex > lastIndex) {
				fragment.append(
					document.createTextNode(sourceText.slice(lastIndex, matchIndex)),
				);
			}
	...
	function clearSearchHighlights(root) {
		...
		root
			.querySelectorAll("[data-roprime-search-highlight]")
			.forEach((element) => {
	...
	function applySearchHighlights(root, term) {
		...
		root.querySelectorAll(highlightTargets.join(", ")).forEach((element) => {
			if (!(element instanceof HTMLElement)) return;
			const sourceText =
				element.dataset.roprimeSearchOriginalText ?? element.textContent ?? "";
			element.dataset.roprimeSearchOriginalText = sourceText;
			const highlightedFragment = buildSearchHighlightFragment(
				sourceText,
				matcher,
			);
	...
	function buildSettingsPaneMarkup() {
        ...
        <div class="${RP_PANEL_CLASS}">
            <h2 class="roprime-settings-page-title" data-i18n="settings.hero.title"></h2>
                ...
                <div class="roprime-settings-main">
                    ...
                    <section class="roprime-settings-section" data-roprime-section="design">
                        <div class="roprime-accordion" data-roprime-accordion="rename">
                            <div class="roprime-accordion-header" role="button" tabindex="0" aria-expanded="false">
                                <div class="roprime-toggle-copy"><div class="roprime-toggle-title" data-i18n="settings.rename.title"></div></div>
                        ...
                        <div class="roprime-toggle-row roprime-sidebar-size-row"><div class="roprime-toggle-copy"><div class="roprime-toggle-title" data-i18n="settings.sidebar.title"></div><div class="roprime-toggle-desc" data-i18n="settings.sidebar.desc"></div></div><div class="roprime-setting-control"><div class="roprime-sidebar-size-control"><div class="roprime-sidebar-size-box"><div class="roprime-sidebar-size-rail"><input id="roprime-sidebar-size-slider" class="roprime-sidebar-size-slider" type="range" min="0" max="100" step="0.1" value="0" aria-label="Sidebar size" /></div><div class="roprime-sidebar-size-ticks"><button class="roprime-sidebar-size-tick" type="button" data-size-mode="full"><span data-i18n="settings.sidebar.full"></span></button><button class="roprime-sidebar-size-tick" type="button" data-size-mode="small"><span data-i18n="settings.sidebar.small"></span></button><button class="roprime-sidebar-size-tick" type="button" data-size-mode="icon"><span data-i18n="settings.sidebar.icon"></span></button></div></div></div></div></div>
                    ...
                    <section class="roprime-settings-section" data-roprime-section="developer" hidden>
                        ...
                        <div class="roprime-setting-field">
                            ...
                            <div class="roprime-setting-control-wrap">
                                <textarea id="roprime-developer-blocked-pages" class="roprime-settings-textarea" rows="6" spellcheck="false" data-i18n-placeholder="settings.developer.blocked.placeholder"></textarea>
                                <button type="button" class="roprime-settings-primary-btn" id="roprime-save-blocked-pages" data-i18n="settings.developer.blocked.save"></button>
                            </div>
                        </div>
                    </section>
                </div>
	...
	function ensureStandaloneSettingsView() {
		...
		if (!standalone) {
			...
			const legacyDescription = standalone.querySelector(
				'[data-roprime-accordion="rename"] .roprime-setting-desc',
			);
			const missingSidebarSizeSlider = !standalone.querySelector(
				"#roprime-sidebar-size-slider",
			);
			const missingOldNavigationBarToggle = !standalone.querySelector(
				"#roprime-toggle-old-navigation-bar",
			);
			const missingFriendStylingReimagnedToggle = !standalone.querySelector(
				"#roprime-toggle-friend-styling-reimagned",
			);
			const missingDeveloperSection = !standalone.querySelector(
				'[data-roprime-section="developer"]',
			);
			if (
				legacyDescription ||
	...
	function bindIndependentDesignToggles(pane, actions, onNavigate) {
		...
		const oldNavigationBarToggle = pane.querySelector(
			"#roprime-toggle-old-navigation-bar",
		);
		const sidebarSizeSlider = pane.querySelector(
			"#roprime-sidebar-size-slider",
		);
		const friendStylingReimagnedToggle = pane.querySelector(
			"#roprime-toggle-friend-styling-reimagned",
		);
		const searchInput = pane.querySelector("#roprime-settings-search");
		const blockedPagesTextarea = pane.querySelector(
			"#roprime-developer-blocked-pages",
		);
		const saveBlockedPagesButton = pane.querySelector(
			"#roprime-save-blocked-pages",
		);
		...
		if (searchInput instanceof HTMLInputElement) {
			...
			const enterSearchMode = () => {
				const isSearchMode =
					pane.getAttribute("data-roprime-search-mode") === "1";
		...
		if (blockedPagesTextarea instanceof HTMLTextAreaElement) {
			blockedPagesTextarea.addEventListener("keydown", (event) =>
				event.stopPropagation(),
			);
		...
		if (
			saveBlockedPagesButton instanceof HTMLButtonElement &&
			blockedPagesTextarea instanceof HTMLTextAreaElement
		) {
			saveBlockedPagesButton.addEventListener("click", () => {
				settingsState.blockedExecutionPages = parseBlockedExecutionPages(
					blockedPagesTextarea.value,
				);
				saveSettings();
				saveBlockedPagesButton.disabled = true;
			});
		}
		const languageDropdown = pane.querySelector(
		...
		if (friendStylingReimagnedToggle instanceof HTMLInputElement) {
			friendStylingReimagnedToggle.addEventListener("change", () => {
				settingsState.friendStylingReimagnedEnabled =
					friendStylingReimagnedToggle.checked;
				saveSettings();
		...
		pane.querySelectorAll(".roprime-settings-nav-btn").forEach((button) => {
			...
			button.addEventListener("click", () => {
				if (
					button.dataset.roprimePage === "developer" &&
					!isDeveloperPageUnlocked()
				)
					return;
	...
	function bindSettingsControls(pane, actions, onNavigate) {
		...
		const communitiesToggle = pane.querySelector(
			"#roprime-toggle-rename-communities",
		);
		const experiencesToggle = pane.querySelector(
			"#roprime-toggle-rename-experiences",
		);
		const marketplaceToggle = pane.querySelector(
			"#roprime-toggle-rename-marketplace",
		);
		...
		const toggleAccordion = () =>
			animateAccordion(!accordion.classList.contains("is-open"));
		const openAccordion = () => {
		...
		masterToggle.addEventListener("pointerdown", (event) =>
			event.stopPropagation(),
		);
		...
		accordionHeader.addEventListener("click", (event) => {
			...
			if (
				...
				target.closest(".roprime-accordion-master-switch")
			)
				return;
			toggleAccordion();
		});
		...
		accordionBody.style.maxHeight = accordion.classList.contains("is-open")
	...
	async function refreshSettingsControls(pane) {
		...
		const developerNavButton = pane.querySelector(
		...
		const isSidebarSizeDragging =
			sidebarSizeSlider instanceof HTMLInputElement &&
			sidebarSizeSlider.getAttribute("data-roprime-dragging") === "1";
		if (sidebarSizeSlider instanceof HTMLInputElement) {
		...
		if (!isSidebarSizeDragging) {
			...
			pane.querySelectorAll(".roprime-sidebar-size-tick").forEach((tick) => {
				...
				tick.classList.toggle(
					"is-active",
					tick.dataset.sizeMode === sidebarSizeMode,
				);
			});
		}
		if (friendStylingReimagnedToggle instanceof HTMLInputElement)
		...
		const searchSourcePage =
			pane.getAttribute("data-roprime-search-source-page") || RP_DEFAULT_PAGE;
		...
		const developerUnlockMessage = pane.querySelector(
			"[data-roprime-developer-unlock-message]",
		);
		if (developerUnlockMessage instanceof HTMLElement) {
			const showUnlockMessage =
				pane.getAttribute("data-roprime-developer-unlock-message-visible") ===
				"1";
			developerUnlockMessage.style.display = showUnlockMessage
				? "block"
				: "none";
		}
		pane.querySelectorAll(".roprime-settings-section").forEach((section) => {
			...
			if (isSearchMode) {
				...
				if (!hasSearchTerm) {
					...
					section
						.querySelectorAll(
							".roprime-toggle-row, .roprime-setting-field, .roprime-accordion, .roprime-info-block",
						)
						.forEach((item) => {
				...
				section
					...
					.forEach((item) => {
						...
						const isMatch =
							hasSearchTerm && isSearchMatch(itemText, searchTerm);
			...
			section.querySelectorAll(".roprime-info-block").forEach((item) => {
				if (!(item instanceof HTMLElement)) return;
				item.style.display = "";
				clearSearchHighlights(item);
			});
			section.style.display =
				...
				!(sectionKey === "developer" && !isDeveloperPageUnlocked())
					? "block"
					: "none";
		});
		const sharedSearchWrap = pane.querySelector(
		...
		if (
			...
			pane
				.querySelectorAll(
					".roprime-settings-section .roprime-toggle-row, .roprime-settings-section .roprime-setting-field, .roprime-settings-section .roprime-accordion, .roprime-settings-section .roprime-info-block",
				)
				.forEach((item) => {
					if (!(item instanceof HTMLElement)) return;
					item.style.display = "";
	...
	function updateSidebarVisibility(hideSidebar) {
		...
		if (!(menu instanceof HTMLElement)) return;
		const sidebarHost =
			menu.closest(".menu-vertical-container") ||
			menu.closest(".rbx-left-col") ||
			menu.parentElement;
	...
	function updateDocumentTitle(showPanel) {
		...
		if (showPanel)
			...
			document.title =
				root.getAttribute("data-roprime-original-title") || document.title;
	}
	...
	function removeRoPrimeDropdownItems() {
		document
			.querySelectorAll(`li.${RP_DROPDOWN_ITEM_CLASS}`)
			.forEach((item) => {
				item.remove();
			});
	}
			...
			for (const mutation of mutations) {
				if (mutation.type === "attributes") {
					if (
						...
						mutation.target.closest("ul.dropdown-menu")
					) {
