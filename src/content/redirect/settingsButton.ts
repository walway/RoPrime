...
import { createSvgIcon } from '../ui/dom.ts'
...
function getOrCreateExtensionsDivider(menuList) {
    const natives = [
        ...
        (li) => !li.hasAttribute(TAB_ENTRY_ATTR) && !li.hasAttribute(EXTENSIONS_TAB_ATTR),
    )
    const anchor = getBrowserPreferencesMenuTab(menuList) ||
