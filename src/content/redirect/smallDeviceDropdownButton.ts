...
import { dismissFoundationWebDropdown } from '../ui/dropdown.ts'
import { appendParsedMarkup } from '../ui/dom.ts'
import { openRoPrimeSettingsOnAccountPage } from '../settings/settingsPage.ts'
...
function syncExtensionsEntryAriaSelected(group, button) {
    ...
    const highlighted = button.hasAttribute('data-highlighted') || button.matches(':hover')
    button.setAttribute(
        'aria-selected',
...
function moveMenuHighlight(group, direction) {
    ...
    const currentIndex = buttons.findIndex((button) => button.hasAttribute('data-highlighted'))
...
function ensureButtonOrder(group) {
    ...
    const insertBefore = roqolButton instanceof HTMLButtonElement ? roqolButton : settingsButton
