...
import { layerIsCurrent, profileUserMayShowEffect, resolveEquippedEffectId } from './effectMount.ts'
import {
    applyProfileEffectIframeTransparentAttrs,
    getProfileEffectById,
    getProfileEffectProfileEmbedSrc,
} from './profileEffectsCatalog.ts'
import { getRobloxUserId, peekRobloxUserId } from './robloxUserId.ts'
import { observeUserCardElements, onUserCardElement } from './userCardElements.ts'
...
function layerClassForContext(context) {
    return context === 'friends-list' ? FRIENDS_LIST_LAYER_CLASS : CAROUSEL_LAYER_CLASS
}
...
function scanCarouselAvatars() {
    for (
        ...
        for (
            const link of container.querySelectorAll(
        ...
        for (
            const avatar of container.querySelectorAll(
