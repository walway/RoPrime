...
import { getRobloxUserId, isRobloxAuthenticated } from './robloxUserId.js'
...
function show2DLoader(host) {
    ...
    shimmer.className = 'thumbnail-2d-container avatar-loading-shimmer-overlay no-background-thumbnail thumbnail-span'
...
async function render2DPreview(localMountSeq, localRenderSeq) {
    ...
    span.className = 'thumbnail-2d-container no-background-thumbnail thumbnail-span'
...
function normalizeAvatarModelResult(raw) {
    const avatarModel = raw?.avatarModel && typeof raw.avatarModel === 'object'
    ...
    const normalizedBackgroundAsset = backgroundAsset && typeof backgroundAsset === 'object'
        ? {
            ...backgroundAsset,
            assetType: normalizeAssetType(backgroundAsset.assetType) ||
    ...
    return {
        ...raw,
        avatarModel,
        avatarConfigurations: raw?.avatarConfigurations && typeof raw.avatarConfigurations === 'object'
            ? {
                ...raw.avatarConfigurations,
                background: raw.avatarConfigurations.background &&
...
function stylePreviewHost(host) {
    host.className = 'roprime-profile-avatar-preview profile-avatar-background-empty-state'
