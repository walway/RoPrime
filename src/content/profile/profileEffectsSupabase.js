...
import { isSupabaseProfileEffectsEnabled, SUPABASE_ANON_KEY, SUPABASE_URL } from './profileEffectsConfig.js'
...
export async function fetchRegistryFromSupabase() {
    ...
    for (const row of ownersResult.data || []) {
        ...
        const purchasedAt = row.purchased_at ? new Date(row.purchased_at).getTime() : Date.now()
...
export async function supabaseRegisterEquip(userId, effectId, kind) {
    ...
    if (existing) {
        patch.picture_effect_id = slot === 'picture_effect_id' ? effectId || null : existing.picture_effect_id
        patch.profile_effect_id = slot === 'profile_effect_id' ? effectId || null : existing.profile_effect_id
    } else {
        patch.picture_effect_id = slot === 'picture_effect_id' ? effectId || null : null
        patch.profile_effect_id = slot === 'profile_effect_id' ? effectId || null : null
    }
