import { getExtensionResourceUrl } from '../core/core.ts'
import { getAllProfileEffectIds } from './profileEffectsCatalog.ts'
import { isSupabaseProfileEffectsEnabled } from './profileEffectsConfig.ts'
import { fetchRegistryFromSupabase, supabaseRegisterEquip, supabaseRegisterPurchase } from './profileEffectsSupabase.ts'

export { isSupabaseProfileEffectsEnabled } from './profileEffectsConfig.ts'
...
export { getRobloxUserId, invalidateRobloxUserIdCache, peekRobloxUserId } from './robloxUserId.ts'
...
export async function getEquippedEffectForProfileUser(
    ...
    const fromMap = equippedByUser && typeof equippedByUser === 'object'
        ? normalizeEquippedEntry(equippedByUser[key])[slot]
        : ''

    const fromLocal = localEquipped && typeof localEquipped === 'object' ? String(localEquipped[slot] || '').trim() : ''
...
export async function syncOwnedEffectsFromRegistry(userId, ownedList) {
    ...
    const registry = await fetchProfileEffectsRegistry()
    const next = new Set(
        Array.isArray(ownedList) ? ownedList.filter((id) => typeof id === 'string' && id.trim()) : [],
    )
