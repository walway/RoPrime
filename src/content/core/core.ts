...
import { collectSupportedPages, collectToggleDefaults } from '../settings/settingsConfig.ts'
import { SIDEBAR_FULL_RESERVED_PX } from '../sidebar/sidebarLayout.ts'
...
export function getAccountSettingsLeftInsetPx(size = getActiveSidebarSize()) {
    ...
    const sidebarWidth = ACCOUNT_SETTINGS_LEFT_INSET_BY_SIZE[mode] ??
...
export function normalizeHiddenSidebarItemsBySize(stored) {
    if (
        ...
        for (const mode of SIDEBAR_SIZE_MODES) {
            ...
            next[mode] = Array.isArray(list) ? list.filter((id) => typeof id === 'string' && id.trim()) : []
        }
        return next
    }
...
export function mergeStoredSettings(stored) {
    ...
    if (stored.renameMarketplaceToCatalog === undefined) {
        if (stored.renameMarketplaceToAvatarShop != null) {
            settingsState.renameMarketplaceToCatalog = !!stored.renameMarketplaceToAvatarShop
        }
    }
    ...
    settingsState.hiddenSidebarItemsBySize = normalizeHiddenSidebarItemsBySize(stored)
...
export function serializeSettingsPayload() {
    return {
        ...
        customCss: typeof settingsState.customCss === 'string' ? settingsState.customCss : '',
        ...
        ownedProfileEffects: Array.isArray(settingsState.ownedProfileEffects)
            ...
            : [],
        equippedProfilePictureEffect: typeof settingsState.equippedProfilePictureEffect === 'string'
            ? settingsState.equippedProfilePictureEffect.trim()
            : '',
        equippedProfilePageEffect: typeof settingsState.equippedProfilePageEffect === 'string'
            ? settingsState.equippedProfilePageEffect.trim()
            : '',
        profileEffectsEquippedByUser: settingsState.profileEffectsEquippedByUser &&
        ...
        robloxFreeThemeClass: typeof settingsState.robloxFreeThemeClass === 'string'
...
export function buildRoPrimeSettingsFullUrl(
    ...
    const base = `${globalThis.location.origin}${getRobloxLocalePathPrefix()}/my/account?${RP_PARAM_KEY}=${
        encodeURIComponent(slug)
    }`
    const h = typeof hashFragment === 'string' && hashFragment.trim()
        ? hashFragment.trim().startsWith('#') ? hashFragment.trim() : `#${hashFragment.trim()}`
        : ''
    return `${base}${h}`
}
