...
import { langList } from '../../i18n/i18n-config.ts'
import { promptCustomCssCautionNotice, promptProfileEffectsSupportNotice } from '../alerts/alert.ts'
import { showMaliciousPluginOverlay, showVersionUpdateOverlay } from '../ui/overlay.ts'
import { detectBrowserDownloadSource, fetchVersionManifest, isInstalledAsPackage } from '../core/version.ts'
...
import { syncSearchBan } from '../features/searchBan.ts'
import { updateDocumentTitle } from '../panel/pageChrome.ts'
import { syncRoPrimeView } from '../panel/panel.ts'
import { hydrateProfilePictureEffectAvatars } from '../profile/profileEffectAvatar.ts'
import {
...
} from '../profile/profileEffectsCatalog.ts'
import {
    equipSlotForKind,
    getRobloxUserId,
...
} from '../profile/profileEffectsRegistry.ts'
import {
    ...
    restoreSidebarItem,
} from '../sidebar/sidebarContent.ts'
import { syncSidebarContent } from '../sidebar/sidebarContent.ts'
import { sidebarItemLabelKey } from '../sidebar/sidebarItemLabels.ts'
import { applyPlainOrRichText } from '../ui/richText.ts'
import { createControlButton, getControlButtonLabel } from '../ui/controlButton.ts'
import { createDropdown } from '../ui/dropdown.ts'
import { setHidden } from '../ui/visibility.ts'
import { createToggle, setToggleChecked } from '../ui/toggle.ts'
import { createMarkedSlider, setSliderDisabled, setSliderValue } from '../ui/slider.ts'
import { t as accountSettingsPaneT } from './translationsHook.ts'
import { clearSettingsPageLayout, resolveSettingsMountHost } from './injectSettingsPage.ts'
import { SETTINGS_CONFIG } from './settingsConfig.ts'
import { createSettingsNavIcon, SETTINGS_NAV_ICONS } from './navBarIcons.ts'
...
function getMobileNavigationOptions() {
    ...
    for (const [pageKey, pageCfg] of Object.entries(SETTINGS_CONFIG)) {
        ...
        options.push({
            value: pageKey,
            label: accountSettingsPaneT(pageCfg.title),
            hidden: pageKey === 'developer' && !unlocked,
        })
    }
    return options
}

function navigateToSettingsPage(nextPage) {
    const root = getSettingsHostRoot()
    if (!(root instanceof HTMLElement)) return
...
function buildSettingsHostContent(host) {
    ...
    const mobileDropdown = buildMobileNavigationDropdown()
    host._rpMobileNavDropdown = mobileDropdown
    settingsContainer.appendChild(mobileDropdown.root)
    ...
    for (const [pageKey, pageCfg] of Object.entries(SETTINGS_CONFIG)) {
        if (pageCfg?.type === 'navDivider') {
            const divider = el('li', 'rbx-divider thick-height')
    ...
    containerV2.append(searchHint, devHint)
...
const SETTINGS_CARD_CLASS = 'bg-surface-300 padding-large flex flex-col gap-medium'
...
function isSettingsSeparator(node) {
    return (
        node instanceof HTMLElement &&
        (node.getAttribute('role') === 'separator' ||
            node.classList.contains('roprime-settings-separator'))
    )
}

function hasVisibleContentBelow(node) {
    ...
    while (next) {
        ...
        if (next.classList.contains('hidden') || next.hasAttribute('hidden')) {
            next = next.nextElementSibling
            continue
        }
        if (isSettingsSeparator(next)) {
            next = next.nextElementSibling
            continue
        }
        const text = (next.textContent || '').trim()
        if (text || next.children.length > 0) return true
        next = next.nextElementSibling
    }
    return false
}
...
function setSettingTitle(node, item) {
    if (!(node instanceof HTMLElement) || !item) return node
    if (item.skipI18n || item.literalTitle) {
        node.textContent = String(item.literalTitle || item.title || '')
        return node
    }
    return setI18n(node, item.title)
}

function createSettingsCardHeaderRow(titleKeyOrItem, trailing = null) {
...
function createSettingsCard({ title, description, trailing, extraContent }) {
    ...
    card.append(createSettingsCardHeaderRow(title, trailing))
    const hasBelow = !!(description || extraContent)
    if (hasBelow) card.appendChild(createSettingsCardHeaderSeparator())
...
function decorateSettingToggle(toggle, item) {
    ...
    if (item.exclusiveWith) {
        toggle.dataset.exclusiveWith = item.exclusiveWith
        toggle.classList.add('roprime-setting-exclusive')
    }
    return toggle
}

function createChildToggleRow(item) {
    const row = el(
        'div',
    ...
    const toggle = createToggle({
        ...
        ariaLabel: item.literalTitle ||
            accountSettingsPaneT(item.title) ||
            String(item.title || item.key),
    })
    decorateSettingToggle(toggle, item)
    row.append(title, toggle)
    return row
}

function createToggleSection(item, children = []) {
    const toggle = createToggle({
        id: item.id || `roprime-toggle-${item.key}`,
        checked: !!settingsState[item.key],
        ariaLabel: item.literalTitle ||
    ...
    if (children.length) {
        ...
        if (item.description) {
            card.appendChild(createSettingsCardDescription(item.description))
        }
        for (const child of children) {
            card.appendChild(createChildToggleRow(child))
        }
        pruneEmptySeparators(card)
        return card
    }
...
function buildConfigCard(cardCfg) {
    ...
    const items = cardCfg.items || []
    if (cardCfg.title) {
        card.append(createSettingsCardHeaderRow(cardCfg.title))
        if (items.length) card.appendChild(createSettingsCardHeaderSeparator())
    }
...
function renderPageContent(container, pageKey) {
    ...
    if (!page || page.type === 'navDivider') return
    const items = page.items || []
    const renderedChildKeys = new Set()

    for (const item of items) {
        if (item.type === 'toggle' && item.parent) {
            if (renderedChildKeys.has(item.key)) continue
            continue
        }
        ...
        if (item.type === 'toggle') {
            const children = items.filter(
                (candidate) => candidate.type === 'toggle' && candidate.parent === item.key,
            )
            for (const child of children) renderedChildKeys.add(child.key)
            node = createToggleSection(item, children)
...
function buildLanguageControl() {
    const dropdown = createDropdown({
        value: currentUiLanguageCode(),
        options: Object.entries(langList).map(([code, label]) => ({
            value: code,
            label,
        })),
        onChange: async (next) => {
            ...
            const root = getSettingsHostRoot()
            if (root instanceof HTMLElement) refreshSettingsUi(root)
            syncAccountSettingsMenuButton()
        },
    })
    dropdown.root.classList.add('roprime-language-dropdown')

    return createSettingsCard({
        title: 'settings.language.title',
        description: 'settings.language.description',
        trailing: dropdown.root,
    })
}
...
function buildProfileEffectCard(effect) {
    ...
    const action = createControlButton('settings.profileEffects.equip')
    action.classList.add('roprime-profile-effect-action')
...
function buildSidebarContentRow(item, mode, isAdd) {
    const row = el(
        'div',
        isAdd ? 'roprime-sidebar-content-row is-removed-item' : 'roprime-sidebar-content-row',
    )
...
function syncEffectButtons(shop) {
    ...
    shop.querySelectorAll('.roprime-profile-effect-action').forEach((btn) => {
        ...
        const label = getControlButtonLabel(btn) || btn
        if (equipped) {
            btn._rpEffectAction = 'unequip'
            setI18n(label, 'settings.profileEffects.unequip')
        } else {
            btn._rpEffectAction = 'equip'
            setI18n(label, 'settings.profileEffects.equip')
        }
...
async function copySettingsExport(root) {
    ...
    setSyncStatus(
        root,
        copied ? accountSettingsPaneT('settings.sync.copied') : accountSettingsPaneT('settings.sync.copyFailed'),
        !copied,
    )
...
function syncLanguageMenuLabels(root) {
    ...
    if (dropdown) {
        ...
        dropdown.setValue(currentUiLanguageCode())
    }
}
    ...
    root.querySelectorAll('.roprime-setting-toggle').forEach((toggleEl) => {
        ...
        button.addEventListener('click', () => {
            ...
            if (
                ...
                exclusiveWith ||
                toggleEl.classList.contains('roprime-setting-exclusive')
            ) {
...
function bindOnce(root) {
    ...
    const navigateToPage = navigateToSettingsPage
    ...
    if (backSidebarContent instanceof HTMLButtonElement) {
        backSidebarContent.addEventListener('click', () => navigateToPage('appearance'))
    }
...
function refreshLayoutAndNav(root) {
    ...
    const mobileDropdown = root._rpMobileNavDropdown ||
        root.querySelector('.mobile-navigation-dropdown')?.roprimeDropdown
    if (mobileDropdown) {
        mobileDropdown.setOptions(getMobileNavigationOptions())
        if (!isSearchMode) {
            mobileDropdown.setValue(activePage)
        }
    }
...
function refreshSettingsUi(root) {
    ...
    root.querySelectorAll('.roprime-setting-toggle').forEach((toggleEl) => {
        ...
        if (parentKey && !settingsState[parentKey]) {
            disabled = true
        }

        toggleEl.classList.toggle('is-renames-disabled', disabled)
        toggleEl.classList.toggle('is-setting-child-disabled', disabled)
        const button = toggleEl.querySelector('button.btn-toggle')
        if (button instanceof HTMLButtonElement) button.disabled = disabled

        const dimRow = toggleEl.closest('.roprime-setting-child-row') ||
            toggleEl.closest('.roprime-sidebar-inline-toggle')
        if (dimRow instanceof HTMLElement) {
            dimRow.classList.toggle('is-setting-row-disabled', disabled)
        }
    })
