...
const SECTION_CLASS =
  "signup-v2-card flex flex-col bg-surface-100 radius-large padding-large";
...
function restyleSubmitButton(loginForm) {
  ...
  const allFilled =
    inputs.length > 0 && inputs.every((input) => String(input.value || "").trim());
...
function applyLoginPageRedesign() {
  ...
  const loginSection =
    document.querySelector(".section-content.login-section") ||
  ...
  for (const label of loginForm.querySelectorAll(
  ...
  for (const altButton of document.querySelectorAll(
    '[class*="otp-login-button"], [class*="cross-device-login-button"]',
  )) {
    if (!(altButton instanceof HTMLElement)) continue;
    const isOtp =
      altButton.id === "otp-login-button" ||
      altButton.classList.contains("otp-login-button");
    const baseIdent = isOtp ? "otp-login-button" : "cross-device-login-button";
    const targetClass = `btn-full-width btn-control-md ${baseIdent} foundation-web-button relative clip group/interactable focus-visible:outline-focus disabled:outline-none cursor-pointer flex items-center justify-center stroke-none padding-y-none select-none radius-medium text-label-medium height-1000 padding-x-medium bg-action-standard content-action-standard shrink-0`;
  ...
  for (const secButton of document.querySelectorAll(
