...
function createPillButton(option, selected) {
    ...
    labelWrap.className = `flex items-center justify-center relative width-full ${
        selected ? 'content-emphasis' : 'content-default'
    } text-label-medium`
...
export function createPill(options = {}) {
    ...
    const buttons = pillOptions.map((option) => createPillButton(option, String(option.value) === String(initialValue)))
...
export function getPillValue(node) {
    ...
    const selected = group.querySelector('[role="radio"][aria-checked="true"]')
    return selected instanceof HTMLButtonElement ? selected.dataset.roprimePillValue ?? null : null
}
...
export function setPillOptions(node, options, selectedValue) {
    ...
    const buttons = options.map((option) => createPillButton(option, String(option.value) === String(current)))
