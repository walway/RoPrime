import { appendParsedMarkup } from './dom.ts'
...
function buildTextFieldMarkup({ id, label, value, focused }) {
    ...
    <div style="display: flex; flex-direction: row; gap: 8px;">
      <div class="MuiFormControl-root MuiFormControl-fullWidth MuiTextField-root css-feqhe6">
        <label class="${labelClasses}" data-shrink="${
        focused ? 'true' : 'false'
    }" for="${inputId}" id="${inputId}-label">${safeLabel}</label>
...
export function createFilter({
    ...
    const getRadioOptions = () => state.options.filter((option) => option.type === 'radio')
    ...
    const renderOptionVisual = (optionButton, option) => {
        ...
        optionButton.setAttribute(
            'aria-label',
            option.type === 'radio' ? isSelected ? 'Hide selected' : 'Show not selected' : ' selected',
        )
        ...
        if (icon instanceof HTMLElement) {
            icon.className = isSelected ? 'icon-radio-check-circle-filled' : 'icon-radio-check-circle'
        }
    }
    ...
    const buildModal = () => {
        ...
        for (const option of state.options) {
            ...
            const icon = createElement(
                'span',
                option.selected ? 'icon-radio-check-circle-filled' : 'icon-radio-check-circle',
            )
