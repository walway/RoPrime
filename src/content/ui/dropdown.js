...
function clearPageInertState() {
    ...
    if (
        ...
        body.style.pointerEvents = body.getAttribute(ROPRIME_BODY_POINTER_ATTR) || ''
...
function removeFocusGuardsIfIdle() {
    ...
    if (openDropdowns.length > 0) return
    for (
        const guard of document.querySelectorAll(
...
function positionPopper(popper, trigger, { popperZIndex = '1050' } = {}) {
    ...
    const top = openBelow ? rect.bottom + gap : rect.top - popperHeight - gap
...
function dispatchEscapeKey(target) {
    const node = target instanceof Element || target instanceof Document ? target : document
    node.dispatchEvent(
...
export function dismissFoundationWebDropdown(origin) {
    ...
    for (
        const wrapper of document.querySelectorAll(
...
export function createDropdown({
    ...
    const triggerWrap = createElement(
        'div',
        includeFormGroup ? 'flex flex-col gap-small form-group' : 'flex flex-col gap-small',
    )
    ...
    const popperMount = popperParent instanceof HTMLElement ? popperParent : document.body
    popperMount.appendChild(popper)

    const optionButtons = []

    const getVisibleOptions = () => state.options.filter((entry) => !entry.hidden)

    const getVisibleButtons = () => optionButtons.filter((button, index) => !state.options[index]?.hidden)
    ...
    const onDocumentPointerDown = (event) => {
        ...
        const inside = root.contains(event.target) || popper.contains(event.target)
        if (state.open) {
            if (inside) return
