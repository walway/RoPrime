import { fetchJsonPreferBackground } from '../core/backgroundFetch.ts'
import {
...
import { isSidebarItemHidden } from './sidebarContent.ts'
import { setHidden } from '../ui/visibility.ts'

export const ROBLOX_EVENTS_CDN_URL = 'https://raw.githubusercontent.com/walway/roprime-data/main/v1/roblox-events.json'
...
function parseEventsJson(raw) {
    const list = Array.isArray(raw) ? raw : Array.isArray(raw?.events) ? raw.events : null
    if (!list) return []
...
function resolveMountTarget(options = {}) {
    const preferOld = options.preferOldNav || settingsState.oldNavigationBarEnabled
