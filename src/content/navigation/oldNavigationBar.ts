...
import { getRobloxUserId } from '../profile/robloxUserId.ts'
import { showOfficialStorePopupLegacy } from '../popups/officialStore/OfficialStorePopupLegacy.ts'
import { syncRobloxEvents } from '../sidebar/robloxEvents.ts'
...
const UNREAD_MESSAGES_API = 'https://privatemessages.roblox.com/v1/messages/unread/count'
const FRIEND_REQUESTS_API = 'https://friends.roblox.com/v1/user/friend-requests/count'
...
function setLeftNavHidden(hidden) {
    ...
    style.textContent = '.left-nav{display:none!important;visibility:hidden!important;}'
...
function buildNavTree(userId) {
    ...
    const profileHref = id ? `${origin()}/users/${id}/profile` : `${origin()}/users/profile`
    const inventoryHref = id ? `${origin()}/users/${id}/inventory` : `${origin()}/users/inventory`
        ...
        el(
            ...
            [
                ...
                el(
                    'span',
                    {
                        className: 'flex flex-col gap-xsmall min-width-0 large:flex-row large:align-items-center',
                    },
                    [
                        el(
                            'span',
...
async function hydrateNav(host, userId, seq) {
    const [displayName, headshotUrl, messages, friends, trades] = await Promise.all([
