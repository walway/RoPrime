...
import { fetchExtensionIconUrl } from '../lib/cws-images.ts'
import { runWhenIdle } from '../features/runWhenIdle.ts'
import { showMaliciousPluginOverlay } from '../ui/overlay.ts'
import { attachTooltip } from '../ui/tooltip.ts'
import { appendParsedMarkup, createSvgIcon } from '../ui/dom.ts'
import { fetchExtensionsRegistry } from './registry.ts'
import { createToggle, getToggleChecked, setToggleChecked } from '../ui/toggle.ts'
...
async function setExtensionIcon(
    ...
    const cwsLookupId = shouldUseRegistryIdForCwsIcon(installType, registryId) ? String(registryId).trim() : id
    ...
    for (const plugin of resp.plugins) {
        ...
        settingsBtn.appendChild(
            createSvgIcon(
                ...
                {
                    viewBox: '0 0 24 24',
                    className: 'MuiSvgIcon-root MuiSvgIcon-fontSizeMedium svg-icon css-o5v4k8',
                },
            ),
        )
...
export function initExtensionsPanel() {
    ...
    document.addEventListener(
        ...
        (event) => {
            ...
            const isNativeFoundationNav = foundationItem instanceof HTMLButtonElement &&
