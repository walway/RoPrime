import { saveSettings, settingsState } from '../core/core.ts'
import { ensureRobloxTranslations, robloxT } from '../core/translations.ts'
import { getRobloxUserId, peekRobloxUserId } from '../profile/robloxUserId.ts'
import { setHidden } from '../ui/visibility.ts'

const ROBLOX_PLUS_MEMBERSHIP_API = 'https://premiumfeatures.roblox.com/v1/users/{userId}/validate-membership'
...
function getSavedThemeClass() {
    const saved = typeof settingsState.robloxFreeThemeClass === 'string'
...
function createTabButton(label, palette) {
    ...
    button.className = activePalette === palette ? TAB_SELECTED_CLASS : TAB_IDLE_CLASS
    if (palette === 'dynamic') {
...
function createThemeCard(theme) {
    ...
    name.className = 'fill basis-0 min-width-0 text-no-wrap text-truncate-end text-body-medium content-default'
...
function buildThemePanel() {
    ...
    description.textContent = 'Choose a theme to customize your experience. Your selection is saved with RoPrime.'
...
function hideNativeAppThemeSections(container) {
    container.querySelectorAll('.flex.flex-col.gap-large').forEach((section) => {
        ...
        const target = settingSection instanceof HTMLElement ? settingSection : section
