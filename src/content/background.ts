import { resolveLocalizedManifestStrings } from './lib/extension-i18n.ts'
...
function normalizeExtensionItem(x) {
    ...
    return {
        ...
        installType: String(x?.installType || '')
            .trim()
            .toLowerCase(),
        hostPermissions: Array.isArray(x?.hostPermissions) ? x.hostPermissions.map((entry) => String(entry || '')) : [],
        permissions: Array.isArray(x?.permissions) ? x.permissions.map((entry) => String(entry || '')) : [],
    }
}
    ...
    try {
        ...
        if (response.ok) {
            ...
            return {
                ...
                description: localized.description || String(manifest?.description || '').trim(),
...
extensionApi.runtime.onMessage.addListener((message, sender, sendResponse) => {
                        ...
                        registry.map(async (entry) => {
                            ...
                            return {
                                ...
                                item: {
                                    ...
                                    description: manifestInfo.description || String(match.description || ''),
    ...
    if (type === 'ROPRIME_FETCH') {
        ...
        fetch(url, {
            ...
            .catch((err) => sendResponse({ ok: false, error: String(err || 'fetch_failed') }))
        return true
    }
