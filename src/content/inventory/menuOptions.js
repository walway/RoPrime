...
function findMenuOptionsWithSecondary(root = document) {
  ...
  const options = [];
  for (const menuOption of scope.querySelectorAll(
...
function collectNativeSubcategories(nativeContainer) {
  const items = [];
  for (const option of nativeContainer.querySelectorAll(
    ...
    items.push({
      ...
      ngRepeat:
        option.getAttribute("ng-repeat") || "subcategory in category.items",
    });
  }
  return items;
}
...
function hideNativeSecondaryContainers(menuOption) {
  ...
  for (const native of menuOption.querySelectorAll(
    `.menu-secondary-container:not([${MARK_ATTR}="1"])`,
  )) {
    if (!(native instanceof HTMLElement)) continue;
    native.hidden = true;
